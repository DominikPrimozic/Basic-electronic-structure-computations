...
void DFT::dipole(){
    //x dipole
    dipoleX=0;
    for (int g=0;g<rho.size();g++){
        dipoleX-=weights[g]*gridx[g]*rho[g];
    }
    dipoleY=0;
    for (int g=0;g<rho.size();g++){
        dipoleY-=weights[g]*gridy[g]*rho[g];
    }
    dipoleZ=0;
    for (int g=0;g<rho.size();g++){
        dipoleZ-=weights[g]*gridz[g]*rho[g];
    }
}

Eigen::VectorXd DFT::nucleardipole(MolecularIntegralsS molint){
    Eigen::VectorXd nudip(3);
    nudip.setZero();
    std::vector<int> Z =molint.molecule.charges;
    std::vector<std::vector<double>> R= molint.molecule.position;
    for (int zz=0;zz<Z.size();zz++){
        for (int cr=0;cr<3;cr++){
            nudip[cr]+=Z[zz]*R[zz][cr];
        }
    }
    return nudip;
}
...
void DFT::quadrupole(){
    Oij.resize(3,3);
    Oij.setZero();
    std::vector<std::vector<double>> gridF(3);
    gridF[0]=gridx;
    gridF[1]=gridy;
    gridF[2]=gridz;
    double value=0;
    for (int g=0;g<rho.size();g++){
        for (int i=0;i<3;i++){
            //for (int j=0;j<3;j++){
            for (int j=0;j<=i;j++){
                value=3*gridF[i][g]*gridF[j][g];
                if (i==j){
                    value-=(gridF[0][g]*gridF[0][g])+(gridF[1][g]*gridF[1][g])+(gridF[2][g]*gridF[2][g]);
                    Oij(i,j)-=value*rho[g]*weights[g] * 1/2; //added
                }
                else{ //added else
                    Oij(i,j)-=value*rho[g]*weights[g] * 1/2;//not sure abotu 1/2s
                    Oij(j,i)-=value*rho[g]*weights[g] * 1/2;
                }
                //Oij(i,j)+=value*rho[g]*weights[g]; //could use symmetry j<=i and i,j=j,i
            }
        }
    }
}

Eigen::MatrixXd DFT::nuclearquadrupole(MolecularIntegralsS molint){
    Eigen::MatrixXd nuqua(3,3);
    nuqua.setZero();
    std::vector<int> Z =molint.molecule.charges;
    std::vector<std::vector<double>> R= molint.molecule.position;
    double value=0;
    for (int zz=0;zz<Z.size();zz++){
        for (int i=0;i<3;i++){
            for (int j=0;j<=i;j++){
                value=3*R[zz][i]*R[zz][j];
                if (i==j){
                    value-=R[zz][0]*R[zz][0]+R[zz][1]*R[zz][1]+R[zz][2]*R[zz][2];
                    nuqua(i,j)+=Z[zz]*value * 1/2; //maybe no 1/2
                }
                else{ 
                    nuqua(i,j)+=Z[zz]*value * 1/2;
                    nuqua(j,i)+=Z[zz]*value * 1/2;
                }
            }
        }
    }
